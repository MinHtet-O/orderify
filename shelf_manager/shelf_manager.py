...
class ShelfManager:
    def __init__(self, pickup_area: PickupArea):
        self.pickup_area = pickup_area

    def manage_shelves(self, event):
        while True:
            event.wait()
    ...
    def __remove_order_on_full(self) -> Order:
        if self.pickup_area.all_shelves_full():
            shelf = self.pickup_area.overflow_shelf
            # remove random order
            index = random.randint(0, len(shelf.size) - 1)
            order = shelf.remove_order(index)
            order.status = OrderStatus.FAILED

    def __discard_spoiled_orders(self):
        orders = self.pickup_area.order_iterator()
        ...
        if pickup_area.overflow_shelf_full() and (not pickup_area.allowable_shelves_full()):
            for index, order in enumerate(pickup_area.overflow_shelf.orders):
                ...
                if not allowable_shelf.full():
                    print("ShelfManager: {} with {} has replaced to allowable shelf".format(order.name, order.temp))
    ...
    def __update_deterioration(self):
        ...
        shelves = self.pickup_area.shelf_iterator()
